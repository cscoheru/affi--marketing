...
// RecordEvent 记录追踪事件
func (s *TrackingService) RecordEvent(ctx context.Context, track *model.Track) error {
	// 生成追踪ID（如果未提供）
	if track.TrackingID == "" {
...
// GetEvent 获取追踪事件详情
func (s *TrackingService) GetEvent(ctx context.Context, id uint) (*model.Track, error) {
...
func (s *TrackingService) GetEventByTrackingID(ctx context.Context, trackingID string) (*model.Track, error) {
	var track model.Track
...
// ListEvents 获取追踪事件列表
func (s *TrackingService) ListEvents(ctx context.Context, filter TrackFilter) ([]model.Track, int64, error) {
	var tracks []model.Track
	var total int64

	query := s.db.WithContext(ctx).Model(&model.Track{})
...
func (s *TrackingService) GetEventsByExperiment(ctx context.Context, experimentID string, limit int) ([]model.Track, error) {
	var tracks []model.Track
...
func (s *TrackingService) GetEventsByUser(ctx context.Context, userID string, limit int) ([]model.Track, error) {
	var tracks []model.Track
...
func (s *TrackingService) GetConversionEvents(ctx context.Context, experimentID string) ([]model.Track, error) {
	var tracks []model.Track
...
func (s *TrackingService) GetTouchpoints(ctx context.Context, conversionID string) ([]model.Touchpoint, error) {
	// 先获取转化事件
	var conversion model.Conversion
	if err := s.db.WithContext(ctx).Where("id = ?", conversionID).First(&conversion).Error; err != nil {
		return nil, fmt.Errorf("conversion not found: %w", err)
	}

	// 获取该转化前的所有触点
	var touchpoints []model.Touchpoint
	if err := s.db.WithContext(ctx).
		Where("track_id = ? AND timestamp <= ?", conversion.TrackID, conversion.CreatedAt).
		Order("timestamp ASC").
...
func (s *TrackingService) RecordConversion(ctx context.Context, conversion *model.Conversion) error {
	// 生成追踪ID（如果未提供）
	...
	// 验证实验是否存在
	var exp model.Experiment
	if err := s.db.WithContext(ctx).Where("id = ?", conversion.ExperimentID).First(&exp).Error; err != nil {
		return fmt.Errorf("experiment not found: %w", err)
	}
...
// GetStats 获取追踪统计
func (s *TrackingService) GetStats(ctx context.Context, experimentID string) (*TrackStats, error) {
	stats := &TrackStats{}
