...
export default function BlogAdminPage() {
  const { articles, fetchArticles, publishArticle, deleteArticle, loading, stats } = useBlogStore()
  ...
  const filteredArticles = (() => {
    switch (activeTab) {
      ...
      case 'review':
        return articles.filter(a => a.status === 'review')
      case 'published':
      ...
      <div className="grid gap-4 md:grid-cols-4 mb-8">
        <div className="rounded-lg border bg-card p-4">
          <div className="flex items-center gap-2 text-muted-foreground mb-1">
            ...
            <span className="text-sm">总文章</span>
          </div>
        ...
        <div className="rounded-lg border bg-card p-4">
          ...
          <p className="text-2xl font-bold text-blue-600">{stats?.reviewArticles || 0}</p>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="grid w-full grid-cols-4 max-w-md">
          <TabsTrigger value="all">
            全部 ({stats?.totalArticles || 0})
          </TabsTrigger>
          <TabsTrigger value="drafts">
            草稿 ({stats?.draftArticles || 0})
          </TabsTrigger>
          <TabsTrigger value="review">
            审核中 ({stats?.reviewArticles || 0})
          </TabsTrigger>
          <TabsTrigger value="published">
            已发布 ({stats?.publishedArticles || 0})
          </TabsTrigger>
        </TabsList>
        <TabsContent value="all">{renderArticleList()}</TabsContent>
        <TabsContent value="drafts">{renderArticleList()}</TabsContent>
        <TabsContent value="review">{renderArticleList()}</TabsContent>
        <TabsContent value="published">{renderArticleList()}</TabsContent>
      </Tabs>
